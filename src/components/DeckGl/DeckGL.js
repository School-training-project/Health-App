...
export default function DeckGL1({
  data,
  ...
  const getTooltip = useCallback((info) => getTooltipText(stationMap, info), [
    stationMap,
  ]);
          ...
          renderSubLayers: (props) => {
            return new GeoJsonLayer(props, {
              ...
              getLineColor: (f) => {
                ...
                return type === "AM"
                  ? amColorScale(frequency)
                  : fmColorScale(frequency);
              },
...
export function renderToDOM(container) {
  render(<DeckGL1 />, container);

  load(DATA_URL.STATIONS, CSVLoader, {
    csv: { delimiter: "\t", skipEmptyLines: true },
  }).then((data) => {
    render(<DeckGL1 data={data} />, container);
  });
}
